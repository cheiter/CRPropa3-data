...
eV = 1.60217657e-19
gamma = logspace(6, 14, 81)  # tabulated UHECR Lorentz-factors


# cross sections for A >= 12 (TALYS)
ddir2 = 'tables/PD_Elastic/new_lower_resolution/'
...
eps2 = iR.romb_pad_logspaced(x, 129)  # padding
xs2elastic = array([iR.romb_pad_zero(x, 129) for x in d2elastic['xs']])*1e-31
...
for field in fields:
    ...
    # save elastic
    fname = ddir2+'pd_elastic_lower_res_%s.txt' % field.name
    output = r_[
    ...
    fname = ddir2+'pd_elastic_lower_res_CDF_%s.txt' % field.name
